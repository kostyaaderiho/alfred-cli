...
class Init {
    ...
    processFiles(passOptions) {
        this.updatePackageJSONFile(passOptions);
        this.updateProjectCLIFile(passOptions);
        this.updateReadmeFile(passOptions);
        this.updatePOMFile(passOptions);
    }
    ...
    async run() {
        ...
        if (!this.options.skipPrompt) {
            passOptions = await this.promtQuestions();
        }
